...
use crate::sync::manifest::{
    ChangeBatch, ChangeOperation, EncryptionConfig, Manifest, ManifestDevice, Snapshot,
};
...
fn operation_associated_data(operation: &ChangeOperation) -> Vec<u8> {
    format!(
        "torder-sync-v1|{}|{}|{}|{}|{}",
        operation.id,
        operation.entity,
...
fn validate_manifest(manifest: &Manifest) -> RepositoryResult<()> {
...
pub fn resolve_conflict_with_payload(
    ...
    let operation = ChangeOperation {
        ...
        base_revision,
        revision,
        changed_at: Utc::now().to_rfc3339_opts(SecondsFormat::Millis, true),
        payload,
    };
...
fn encode_snapshot(snapshot: &Snapshot) -> RepositoryResult<Vec<u8>> {
...
fn decode_snapshot(payload: &[u8]) -> RepositoryResult<Snapshot> {
...
fn apply_batch(connection: &mut rusqlite::Connection, batch: &ChangeBatch) -> RepositoryResult<()> {
...
fn validate_operation(operation: &ChangeOperation) -> RepositoryResult<()> {
    if !matches!(
...
fn validate_json_limits(value: &Value, depth: usize) -> RepositoryResult<()> {
...
fn boolean_field(payload: &serde_json::Map<String, Value>, key: &str) -> RepositoryResult<()> {
...
fn validate_weekdays(value: Option<&Value>) -> RepositoryResult<()> {
...
fn timestamp_field(payload: &serde_json::Map<String, Value>, key: &str) -> RepositoryResult<()> {
    let Some(value) = payload.get(key) else {
        return Ok(());
    };
    if value.is_null()
...
fn date_field(payload: &serde_json::Map<String, Value>, key: &str) -> RepositoryResult<()> {
    let Some(value) = payload.get(key) else {
        return Ok(());
    };
    let value = value
