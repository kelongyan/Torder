...
export function TaskListView({
  ...
  if (loading && tasks.length === 0) {
    return (
      <div className="glass-surface body-copy flex min-h-72 items-center justify-center rounded-2xl">
    ...
    <div className="glass-surface overflow-hidden rounded-2xl">
      {tasks.map((task) => {
        ...
        return (
          <article
            key={task.id}
            className="glass-row group grid min-h-16 grid-cols-[auto_minmax(0,1fr)] items-center gap-3 border-b last:border-b-0"
          >
            <button
              type="button"
            ...
            <button
              type="button"
              onClick={() => onOpen(task)}
              className="min-w-0 py-3 pr-3 text-left focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-emerald-800 dark:focus-visible:outline-blue-300"
            >
              <span
                className={`block truncate text-[15px] leading-[22px] font-medium ${
                  task.status === "done"
              ...
              <span className="meta-copy mt-1 flex flex-wrap items-center gap-x-3 gap-y-1">
                {dueLabel && (
                  <span
