...
- **Database**: `%APPDATA%\com.zhaxideler.torder\torder.sqlite`. Auto-migrated on startup. Schema version is derived from `CURRENT_SCHEMA_VERSION` in `migrations.rs` (the last entry in `MIGRATIONS`). Only append migrations — never edit history. Tests and backup validation should reference the constant instead of duplicating a version literal.
